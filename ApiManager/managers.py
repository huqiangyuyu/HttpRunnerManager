...
class TestCaseInfoManager(models.Manager):
    ...
    def get_case_id(self, name,project):
        return self.filter(belong_project__exact=project).get(name=name).id
...
class ApiInfoManager(models.Manager):
    ...
    def get_api_name(self, name, module_name, belong_project):
        return self.filter(belong_module__id=module_name).filter(name__exact=name).filter(
            belong_project__exact=belong_project).count()
    ...
    def query_api_name(self, name,module, belong_project):
        # return self.get(name=name).filter(belong_module=module).id
        return self.filter(
            belong_project__exact=belong_project).filter(belong_module=module).filter(name=name).values('request')

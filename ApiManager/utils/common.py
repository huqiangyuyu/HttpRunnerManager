	...
	if not isinstance(kwargs, dict) or not kwargs:
		...
		for value in test:
			...
			#         lists.append(value.get('value'))
				...
				lists.append(value)
		if keyword == 'extract' or keyword == 'variables':
			return dict_data
		else:
			return lists
...
def load_cases(type=1, **kwargs):
	...
	if module == '请选择':
		return ''
	api_info = TestCaseInfo.objects.filter(belong_project=belong_project, belong_module=module, type=type). \
		values_list('id', 'name').order_by('-create_time')
	api_info = list(api_info)
	string = ''
	for value in api_info:
		string = string + str(value[0]) + '^=' + value[1] + 'replaceFlag'
	return string[:len(string) - 11]

def load_api(type=1, **kwargs):
	"""
	加载指定项目模块下的接口
	:param kwargs: dict: 项目与模块信息
	:return: str: 用例信息
	...
	if module == '请选择':
		return ''
	case_info = ApiInfo.objects.filter(belong_project=belong_project, belong_module=module, type=type). \
		values_list('id', 'name').order_by('-create_time')
...
def api_info_logic(type=True, **kwargs):
	...
	if 'request' not in test.keys():
		...
		if test.get('name').get('api_name') is '':
			return '接口名称不可为空'
		if test.get('name').get('module') == '请选择':
			return '请选择或者添加模块'
		if test.get('name').get('project') == '请选择':
		...
		name = test.pop('name')
		test.setdefault('name', name.pop('api_name'))

		test.setdefault('api_info', name)
		...
		kwargs.setdefault('teststeps', test)
	...
	for file in files:
		...
		if 'config' in content.keys():
			...
			for i in range(len(case_dict['teststeps'])):
				...
				api = case_dict['teststeps'][i]['api']
				api_name = re.findall('./(.*)',re.findall('./(.*).yml',api)[0])
				api_id = query_api_id(api_name[0],module)
...
def update_include(include):
	...
	for i in range(0, len(include)):
		if isinstance(include[i], dict):
			...
			try:
				if tag == 1:
					name = ApiInfo.objects.get(id=id).name
				else:
					name = TestCaseInfo.objects.get(id=id).name
			except ObjectDoesNotExist:
